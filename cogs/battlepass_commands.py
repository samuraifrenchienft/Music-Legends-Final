...
class BattlePassCommands(commands.Cog):
    ...
    async def quests_command(self, interaction: Interaction):
        self._ensure_season_table()
        ...
        with sqlite3.connect(self.db.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT quest_progress, last_quest_reset FROM season_progress WHERE user_id = ?",
                (interaction.user.id,))
            row = cursor.fetchone()
        ...
        if last_reset != today:
            ...
            with sqlite3.connect(self.db.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT INTO season_progress (user_id, quest_progress, last_quest_reset)
                    VALUES (?, '{}', ?)
                    ON CONFLICT(user_id) DO UPDATE SET quest_progress = '{}', last_quest_reset = ?
                """, (interaction.user.id, today, today))
                conn.commit()

        embed = discord.Embed(
            title="📋 Daily Quests",
            description="Complete quests to earn gold and XP!",
            color=0xf39c12,
        )

        for quest_id, quest in DAILY_QUESTS.items():
            current = progress.get(quest_id, 0)
            # DAILY_QUESTS from config/economy.py has description, reward_gold, reward_xp
            desc = quest.get('description', quest_id)
            gold_r = quest.get('reward_gold', 100)
            xp_r = quest.get('reward_xp', 25)

            # Quest requirement — infer from quest_id
            req_map = {'battle_3': 3, 'collect_5': 5, 'trade_1': 1}
            requirement = req_map.get(quest_id, 1)

            done = current >= requirement
            status = "✅" if done else f"{current}/{requirement}"
            embed.add_field(
                name=f"{'✅' if done else '⬜'} {desc}",
                value=f"Progress: **{status}**\nReward: {gold_r}g + {xp_r} XP",
                inline=False,
            )
