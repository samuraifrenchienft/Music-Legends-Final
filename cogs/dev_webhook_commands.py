...
class DevWebhookCommandsCog(commands.Cog):
    ...
    @app_commands.command(name="sample_cosmetic_card", description="[DEV] Post sample customized cards to dev channel")
    async def sample_cosmetic_card(self, interaction: Interaction):
        """Post sample cosmetic cards showing customization options"""
        if not await check_and_respond(interaction):
            return
        ...
        try:
            # Sample cards with different customizations
            sample_cards = [
                {
                    "name": "The Weeknd",
                    "title": "Blinding Lights",
                    "rarity": "legendary",
                    "frame_style": "holographic",
                    "foil_effect": "rainbow",
                    "impact": 92, "skill": 88, "longevity": 90, "culture": 95, "hype": 93
                },
                {
                    "name": "Dua Lipa",
                    "title": "Levitating",
                    "rarity": "epic",
                    "frame_style": "neon",
                    "foil_effect": "prismatic",
                    "impact": 85, "skill": 82, "longevity": 80, "culture": 88, "hype": 87
                },
                {
                    "name": "Drake",
                    "title": "Hotline Bling",
                    ...
                    "frame_style": "crystal",
                    "foil_effect": "galaxy",
                    "impact": 90, "skill": 93, "longevity": 88, "culture": 91, "hype": 94
                }
            ]
            
            # Frame and effect displays
            frame_display = {
            ...
            for card in sample_cards:
                ...
                embed = discord.Embed(
                    ...
                    description=f"**{card['title']}** • {card['rarity'].upper()}",
                    color=rarity_colors.get(card['rarity'], discord.Color.greyple())
                )
                
                embed.add_field(
                    name="🎨 Customization",
                    value=f"**Frame:** {frame_display.get(card['frame_style'], 'Standard')}\n"
                          f"**Effect:** {foil_display.get(card['foil_effect'], 'Standard')}",
                    inline=False
                )
                
                embed.add_field(
                    name="📊 Stats",
                    value=f"**Impact:** {card['impact']}\n"
                          f"**Skill:** {card['skill']}\n"
                          f"**Longevity:** {card['longevity']}\n"
                          f"**Culture:** {card['culture']}\n"
                          f"**Hype:** {card['hype']}\n"
                          f"**Total Power:** {total_power}",
                    inline=False
                )
                
                embed.set_footer(text="Sample Cosmetic Card - Customization Demo")
                
                # Send to current channel
                await interaction.channel.send(embed=embed)
                print(f"✅ Posted sample card: {card['name']}")
            
            await interaction.followup.send(
                f"✅ Posted {len(sample_cards)} sample cosmetic cards!",
                ephemeral=True
            )
            
        except Exception as e:
            print(f"❌ Error posting sample cards: {e}")
            import traceback
            traceback.print_exc()
            await interaction.followup.send(f"❌ Error: {e}", ephemeral=True)
